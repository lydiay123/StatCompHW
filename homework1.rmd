...
```{r}
mod <- lm(los ~ gender + age + smoker + exercise, dat=x)
summary(mod)
```
...
```
Gender seem to have the highest effect on 'los' given that its coefficient is 4.508675 and it as the smallest P-value.  
```
...
```{r}
coef(summary(mod))
2*pt(coef(summary(mod))[2,3], df = 7, lower.tail = FALSE)
```
...
```
Yes, when 'var.equal = TRUE', the p value for gender equals tthe produced output.
